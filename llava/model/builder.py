...
def load_pretrained_model_monst3r(model_path, model_base, model_name, torch_dtype=torch.bfloat16, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    kwargs = {"device_map": device_map, **kwargs}
    ...
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
            print('Loading LLaVA from base model...')
            model = Llava3dForMonst3r.from_pretrained(model_base, low_cpu_mem_usage=True, config=lora_cfg_pretrained, **kwargs)
            ...
            print('Loading additional LLaVA weights...')
            mm_projector_weights = torch.load(os.path.join(model_path, 'non_lora_trainables.bin'), map_location='cpu')
            mm_projector_weights = {k.replace("base_model.model.",""): v.to(torch.bfloat16) for k, v in mm_projector_weights.items()}
            ...
            # print("missing_keys", keys_not_loaded.missing_keys)
            from peft import PeftModel
            print('Loading LoRA weights...')
            model = PeftModel.from_pretrained(model, model_path)
            print('Merging LoRA weights...')
            ...
            if 'mpt' in model_name.lower():
                ...
                model = Llava3dForMonst3r.from_pretrained(model_base, low_cpu_mem_usage=True, config=cfg_pretrained, **kwargs)

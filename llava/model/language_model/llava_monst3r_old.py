...
class VisionResampler(nn.Module):
    ...
    def __init__(self, identity_embedding_dim=OBJECT_ID_EMBD_SIZE, hidden_dim=1024, dropout=0.1):
        ...
        elif self.ABLISION_STUDY == 'attention':
            # 使用 attention 进行融合
            # self.attn_dim = 256
            self.attn_dim = 400
            self.position_embd_proj = nn.Linear(self.clip_dim, self.attn_dim)  # 3D position embedding
    ...
    def post_init(self):
        ...
        # 手动初始化所有模块，防止from_pretrained时出现问题
    ...
    def sinusoidal_time_embedding(self, t, dim):
        ...
        Args:
            ...
            dim: int - 嵌入维度，通常为 1024
        Returns:
            Tensor of shape (total_timestamps, 1024) - Sinusoidal 编码
        """
        device = t.device
        dtype = t.dtype
        position = t.unsqueeze(-1)  # (total_timestamps, 1)
    ...
    def encode_valid_timestamps(self, valid_timestamps, device, dtype, dim):
        ...
        flat_timestamps = valid_timestamps.values()  # (total_timestamps,)
        ...
        # 生成原始点索引
        lengths = torch.tensor([ts.size(0) for ts in valid_timestamps], device=device)  # (num_points,)
        point_indices = torch.arange(num_points, device=device).repeat_interleave(lengths)  # (total_timestamps,)
        ...
        fake_one = torch.ones_like(point_indices, dtype=torch.long)  # 用于辅助+1

        time1 = time()
        # 在循环外进行全局去重
        unique_timestamps, inverse_indices = torch.unique(flat_timestamps, return_inverse=True)
        sinusoid_unique = self.sinusoidal_time_embedding(unique_timestamps, dim)  # (num_unique, 1024)
        assert inverse_indices.size(0)==point_indices.size(0), f"indices size not match {inverse_indices.size(0)} != {point_indices.size(0)}"

        # 逐个 unique timestamp 聚合，逻辑上与上面的分块处理相同
        for i in range(unique_timestamps.size(0)):
            mask = (inverse_indices == i)  # 当前 unique timestamp 对应的时间戳位置
            curr_point_indices = point_indices[mask]  # 当前时间戳对应的点索引
            if curr_point_indices.numel() > 0:
                curr_embedding = sinusoid_unique[i].unsqueeze(0)  # (1, 1024)
                ...
                total_lengths.index_add_(0, curr_point_indices, fake_one[:curr_point_indices.size(0)])  # 如果索引相同，则多次加1
        time2 = time()
        # print(f"Time for chunking: {time2 - time1:.2f}s")
        ...
        # 融合最大池化和平均池化
        fused_emb = self.fusion_alpha * max_emb + (1 - self.fusion_alpha) * avg_emb  # (num_points, 1024)
        aggregated_time_embs = F.normalize(fused_emb, dim=-1)  # (num_points, 1024)
        # 检查 NaN
        assert not torch.isnan(aggregated_time_embs).any(), "NaN detected!"
        return aggregated_time_embs
    ...
    def forward(self, valid_obj_embedding, valid_timestamps, pos_feat):
        ...
        if self.ABLISION_STUDY == 'no_time':
            object_features = self.encode_object_id(valid_obj_embedding)
            object_features += pos_feat  # 融合位置特征

        elif self.ABLISION_STUDY == 'no_obj':
            object_features = self.encode_valid_timestamps(valid_timestamps, device, dtype, dim=self.clip_dim)
            object_features += pos_feat
            ...
            fusion_input = torch.cat([obj_feat, time_feat], dim=-1)
            object_features = self.fusion_mlp(fusion_input)
            object_features = F.normalize(object_features, dim=-1)
            object_features += pos_feat
            ...
            # in_proj_weight = self.fusion_attention.in_proj_weight
            # in_proj_bias = self.fusion_attention.in_proj_bias
            # assert not torch.isnan(in_proj_weight).any(), "NaN detected in in_proj_weight!"
            # if in_proj_bias is not None:
            #     assert not torch.isnan(in_proj_bias).any(), "NaN detected in in_proj_bias!"

            pos_feat = F.normalize(pos_feat, dim=-1)
            object_features = self.process_attention_and_proj(pos_feat, obj_feat, time_feat)
            object_features = F.normalize(object_features, dim=-1)

        elif self.ABLISION_STUDY == 'no_time_or_obj':
            return pos_feat  # 仅使用位置特征

        else:
            raise ValueError(f"Invalid ABLISION_STUDY value: {self.ABLISION_STUDY}")

        assert not torch.isnan(object_features).any(), "NaN detected!"
        return object_features
        
    
    def process_attention_and_proj(self, pos_feat, obj_feat, time_feat) -> torch.Tensor:
        """
        Returns:
            object_features: 形状 (N, D)
        """

        pos_feat = self.position_embd_proj(pos_feat) # (N, D)
        # 改为拼接后的自注意力
        context_feat = torch.stack([pos_feat, obj_feat, time_feat], dim=1)  # (N, 3, D)
        ...
        attn_output = attn_output.flatten(1)  # (N, 3*D)
        attn_output = self.post_attn_proj(attn_output)  # (N, 1024)
        return attn_output
...
class Llava3dForMonst3r(LlamaForCausalLM, LlavaMetaForCausalLM):
    ...
    def handle_3d_input(self, voxel_grid: dict, max_tokens=2560):
        pc_point, pc_feature = [], []
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        pc_features = []
        for img_timestamps_ns, valid_features, valid_points, valid_obj_embedding, valid_timestamps, traj_prompt, octree_indice, in zip(
            videos_timestamps, videos_points_features, videos_points, videos_objects_embeddings, videos_points_timestamps, trajactory_prompts, voxel_indices, strict=True,
        ):
            ...
            #   ...
            # }
            pc_feature = self.handle_3d_input(voxel_grid, max_tokens=1600)  # 3D input batch_size*[num_points, 4096]

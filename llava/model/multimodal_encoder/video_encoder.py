...
class RGBDVideoTower(nn.Module):
    ...
    def forward(self, features, depths, poses, intrinsics, lengths=None):
        ...
        # feat_xyz torch.Size([1, 20, 24, 24, 3]), xyz torch.Size([1, 20, 336, 336, 3])
        print("original min feat_xyz", feat_xyz.flatten(0, 3).min())
        print("original max feat_xyz", feat_xyz.flatten(0, 3).max())

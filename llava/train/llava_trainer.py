...
def safe_save_model_for_hf_trainer(trainer: Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):  # 对于 pretraining 来说我们只需要保存 projection layer 
        # Only save Adapter
        keys_to_match = ['mm_projector', 'video_tower', 'vision_resampler',]
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            if current_folder.startswith('checkpoint-'):
                ...
                print(f"Saving {len(weight_to_save)} keys to {mm_projector_folder}")
                torch.save(weight_to_save, os.path.join(mm_projector_folder, f'{current_folder}.bin'))
            else:
                print(f"Saving {len(weight_to_save)} keys to {output_dir}")
...
class LLaVATrainer(Trainer):
    ...
    def create_accelerator_and_postprocess(self):
        ...
        print("Setting NCCL timeout to INF to avoid running errors.")
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            ...
            # Only save Adapter
            keys_to_match = ['mm_projector', 'vision_resampler', 'video_tower']
            ...
            if self.args.local_rank == 0 or self.args.local_rank == -1:
                ...
                # **新增：如果模型是 LoRA 适配的，保存 LoRA 权重**
                if isinstance(self.model, LoraModel) or hasattr(self.model, "peft_config"):
                    print(f"Saving LoRA adapter weights to {output_dir}")
                    self.model.save_pretrained(output_dir)  # 这会自动保存 LoRA adapter   

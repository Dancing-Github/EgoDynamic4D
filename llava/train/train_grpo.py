...
export PYTORCH_CUDA_ALLOC_CONF='expandable_segments:True' TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC=3600 TORCH_NCCL_ENABLE_MONITORING=0 NCCL_IB_TIMEOUT=22
torchrun --nproc_per_node=8 \
    ...
    --weight_decay 0. \
    --warmup_ratio 0.03 \
    --output_dir=./grpo_no_time_or_obj \
...
from llava.train.grpo_trainer import (
...
class TrainingArguments(GRPOConfig):
    ...
    epsilon=0.2,
    num_iterations=1,
    reward_clip_range=(-10, 10),  # optional
...
class LazySupervisedDataset(Dataset):
    ...
    def load_json(self, data_folder, qa_per_item=1, max_conv_len=2500):
        list_data_dict = []
        all_token_length = []
        rank0_print(f"Loading data from {data_folder}")
        ...
        for json_path in train_folder.glob('*.json'):
            ...
            npz_path = Path(data_folder, "clip") / json_path.name.replace('_qa_pairs.json', '_scene_data.npz')
    ...
    def load_scene(self, scene_file, max_cache_size=50):
        ...
        scene = (videos_timestamps, videos_points_features, videos_points,
                videos_objects_embeddings, videos_points_timestamps, trajactory_prompts, voxel_indices)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(i, int):
            data_dict = dict(
                ...
                conversations_text=data_dict["conversations"][0],
        ...
        if 'video' in self.list_data_dict[i]:
            ...
            (
            videos_timestamps,videos_points_features,videos_points,
            videos_objects_embeddings,videos_points_timestamps,trajactory_prompts,voxel_indices
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        batch['conversations_text'] = [instance["conversations_text"] for instance in instances]
...
# 奖励函数：解析生成文本与 GT answer 提取坐标，计算欧几里得距离
...
def reward_func(prompts, completions, completion_ids, **kwargs):
    ...
    answers = kwargs.get("answers", [])  # GT answer 文本
    rewards = []
    for gen, ans in zip(completions, answers):
        pred = extract_xyz(gen)
        gt = extract_xyz(ans)
        if pred is None or gt is None:
            rewards.append(None)
        else:
            dist = np.sqrt(np.sum((a - b) ** 2 for a, b in zip(pred, gt)))
            rewards.append(-dist)
    return [0.5]*len(completions)  # 返回一个固定的奖励值列表，实际应用中应根据具体任务计算奖励
...
def train(attn_implementation=None):
    ...
    if checkpoint_paths:
        ...
        mm_projector_weights = torch.load(Path(model_path, 'mm_projector.bin'), map_location='cpu')
